...
def test_audio_button_is_always_added():
    markup = build_resolution_keyboard("req1", [1080, 720], "ru", sizes={1080: 50_000_000})
...
def test_audio_button_text_has_no_size_or_format_details():
    ...
    markup = build_resolution_keyboard("req1", [1080], "ru", sizes={1080: 50_000_000})
    audio_button = next(b for b in _flat_buttons(markup) if b.callback_data == "dla:req1")
    assert audio_button.text == "🎵 Скачать аудио"


def test_audio_button_text_english():
    markup = build_resolution_keyboard("req1", [1080], "en", sizes={1080: 50_000_000})
    audio_button = next(b for b in _flat_buttons(markup) if b.callback_data == "dla:req1")
    assert audio_button.text == "🎵 Download audio"


def test_resolution_buttons_unaffected_by_audio_button():
    markup = build_resolution_keyboard(
        "req1", [1080, 720], "ru", sizes={1080: 50_000_000, 720: None}
    )
    buttons = _flat_buttons(markup)
...
def test_resolution_button_size_label_respects_language():
    markup = build_resolution_keyboard("req1", [1080], "en", sizes={1080: 50_000_000})
    button = next(b for b in _flat_buttons(markup) if b.callback_data == "dl:req1:1080")
    assert "MB" in button.text
    assert "МБ" not in button.text
