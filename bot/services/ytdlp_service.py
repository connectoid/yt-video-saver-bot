...
def _base_ydl_opts(cookies_file: Path | None = None) -> dict:
    ...
    # реально передаётся.
    #
    # android_vr — ДОПОЛНИТЕЛЬНЫЙ клиент в конце списка, всегда. Найдено
    # диагностикой на проде 2026-08-31 (см. README): для видео, где
    # YouTube форсирует SABR-стриминг и tv/web_safari/web отдают ТОЛЬКО
    # storyboard-картинки без единого настоящего формата (см. yt-dlp issue
    # #12482), android_vr всё равно отдаёт хотя бы один прогрессивный
    # поток (обычно 360p, видео+звук вместе) — БЕЗ PO-токена и БЕЗ кук,
    # проверку "Sign in to confirm" тоже не показывает. yt-dlp с списком
    # клиентов запрашивает их все и объединяет форматы — поэтому это
    # только подстраховка (страховочный минимум качества), не замена
    # основным клиентам: если tv/web_safari/web дали полный список
    # форматов, android_vr просто добавит с список формат снизу, который
    # обычно проигрывает по битрейту/высоте и не будет выбран. Известное
    # ограничение: раз это единственный формат для сильно ограниченных
    # видео, кнопка "Скачать аудио" (bestaudio/best) в таком случае тоже
    # скачает этот же файл целиком (видео+звук) — отдельной аудио-дорожки
    # у android_vr без PO-токена нет. Компромисс осознанный: лучше видео
    # похуже или "аудио" с картинкой, чем жёсткий отказ.
    player_clients = ["web_safari", "web", "android_vr"] if cookies_file else ["tv", "web_safari", "android_vr"]
    opts: dict = {
        "quiet": True,
        "no_warnings": True,
        "noplaylist": True,
